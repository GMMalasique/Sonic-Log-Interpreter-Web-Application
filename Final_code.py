...
with tab3:
    ...
    if mode == 'Use sample LAS file':
        file = r"WLC_PETRO_COMPUTED_INPUT_1.LAS"
      ...
      if lascheck_mode:
        ...
        try:
            ...
            if las.check_conformity() == False:
                st.write("The following problem in occured the LAS file:")
    ...
    # Display the DataFrame as a presentable Excel-like table
      ...
      if visualization_log_mode:    
        ...
        with open(pdf_filename, "rb") as pdf_file:
            st.download_button("Download PDF", pdf_file.read(), key="pdf_button", mime="application/pdf")
...
with tab5:
    img_diones = Image.open("Diones.jpeg")
    img_guillo = Image.open("Guillo.jpeg")
    img_malasique = Image.open("Malasique.jpeg")
    img_diones = img_diones.resize([int(100), int(100)])
    img_guillo = img_guillo.resize([int(100), int(100)])
    img_malasique = img_malasique.resize([int(100), int(100)])
    st.image(img_diones)
    st.image(img_guillo)
    st.image(img_malasique)
